locals {
  ...
  common_pre_install_k3s_commands = concat(
    ...
    [
      join("\n", [
        ...
        "  ip route add default via ${local.gateway_ipv4} dev eth1 metric 20101 2>/dev/null",
        "  ",
  ...
  # The Hetzner gateway IP address is always the first IP address of the network CIDR block
  gateway_ipv4 = cidrhost(var.network_ipv4_cidr, 1)
